...
# overwight/obese outcome (log-odds link 
...
# sep dat
dat_chi_ovo_sep <- arrow::read_parquet("dat/chi_ovo_sep_dat.parquet")
dat_chi_ovo_sep
...
mod_chi_ovo_sep <- 
  glmmTMB(
    ovo ~ sex + (age_cat + sep + prs)^2 + (1 + waveC | hicid),
    family = binomial(), 
    data = dat_chi_ovo_sep,
    REML = TRUE
    # verbose = TRUE
  )
summary(mod_chi_ovo_sep)
car::Anova(mod_chi_ovo_sep, type = "III")



# output summary to text file
sink("res/mod_chi_ovo_sep.txt")

report_model(mod_chi_ovo_sep); cat(".\n\n")
report_performance(mod_chi_ovo_sep); cat("\n")
report_intercept(mod_chi_ovo_sep); cat("\n\n")
# report_random(mod_chi_ovo_sep)

mod_rep <- report(mod_chi_ovo_sep) # str(mod_rep)
...
arrow::write_parquet(df_preds, sink = "res/mod_ovo_chi_sep_preds.parquet")
...
newdat_base <- dat_chi_ovo_sep 
newdat_base <- newdat_base %>% mutate(sex = 0.5)
newdat_basep <- predict(mod_chi_ovo_sep, newdata = newdat_base, type = "response", allow.new.levels = FALSE, re.form=NULL)
# newdat_basep <- predict(mod_chi_ovo_sep, type = "response", allow.new.levels = TRUE)
...
newdat_base_pred <- 
  predict(
    mod_chi_ovo_sep, 
    newdata = dat_chi_ovo_sep, 
    type = "response", 
    allow.new.levels = FALSE, 
    re.form=NULL
  )
newdat_base <- 
  cbind(dat_chi_ovo_sep, pred_ovo = newdat_base_pred) %>%
  group_by(sep, prs, age_cat, sex) %>%
  summarise(pred_p = mean(pred_ovo), .groups = "drop") %>%
  arrange(sep, prs, age_cat, sex)

# test
get_pred_by_cats(mod_chi_ovo_sep, dat_chi_ovo_sep)
get_pred_by_cats(mod_chi_ovo_sep, dat_chi_ovo_sep) %>%
  full_join(
    .,
    newdat_base,
...
all(
  abs(
    get_pred_by_cats(fit_glmm_tmb(dat_chi_ovo_sep), dat_chi_ovo_sep)$pred_p - 
      newdat_base$pred_p
  ) < 1e-9
)
...
arrow::write_parquet(boot_df_preds, sink = "res/mod_ovo_chi_sep_boot_preds.parquet")
