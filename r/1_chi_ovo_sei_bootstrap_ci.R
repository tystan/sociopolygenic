...
# sei dat
dat_chi_ovo_sei <- arrow::read_parquet("dat/chi_ovo_sei_dat.parquet")
dat_chi_ovo_sei
...
mod_chi_ovo_sei <- 
  glmmTMB(
    ovo ~ sex + (age_cat + sei + prs)^2 + (1 + waveC | hicid),
    family = binomial(), 
    data = dat_chi_ovo_sei,
    REML = TRUE
    # verbose = TRUE
  )
summary(mod_chi_ovo_sei)
car::Anova(mod_chi_ovo_sei, type = "III")



# output summary to text file
sink("res/mod_chi_ovo_sei.txt")

report_model(mod_chi_ovo_sei); cat(".\n\n")
report_performance(mod_chi_ovo_sei); cat("\n")
report_intercept(mod_chi_ovo_sei); cat("\n\n")
# report_random(mod_chi_ovo_sei)

mod_rep <- report(mod_chi_ovo_sei) # str(mod_rep)
...
arrow::write_parquet(df_preds, sink = "res/mod_ovo_chi_sei_preds.parquet")
...
newdat_base <- dat_chi_ovo_sei 
newdat_base <- newdat_base %>% mutate(sex = 0.5)
newdat_basep <- predict(mod_chi_ovo_sei, newdata = newdat_base, type = "response", allow.new.levels = FALSE, re.form=NULL)
# newdat_basep <- predict(mod_chi_ovo_sei, type = "response", allow.new.levels = TRUE)
...
newdat_base_pred <- 
  predict(
    mod_chi_ovo_sei, 
    newdata = dat_chi_ovo_sei, 
    type = "response", 
    allow.new.levels = FALSE, 
    re.form=NULL
  )
newdat_base <- 
  cbind(dat_chi_ovo_sei, pred_ovo = newdat_base_pred) %>%
  group_by(sei, prs, age_cat, sex) %>%
  summarise(pred_p = mean(pred_ovo), .groups = "drop") %>%
  arrange(sei, prs, age_cat, sex)

# test
get_pred_by_cats(mod_chi_ovo_sei, dat_chi_ovo_sei)
get_pred_by_cats(mod_chi_ovo_sei, dat_chi_ovo_sei) %>%
  full_join(
    .,
    newdat_base,
...
all(
  abs(
    get_pred_by_cats(fit_glmm_tmb(dat_chi_ovo_sei), dat_chi_ovo_sei)$pred_p - 
      newdat_base$pred_p
  ) < 1e-9
)
...
arrow::write_parquet(boot_df_preds, sink = "res/mod_ovo_chi_sei_boot_preds.parquet")
