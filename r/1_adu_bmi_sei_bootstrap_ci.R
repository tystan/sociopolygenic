...
# sei dat
dat_adu_bmi_sei <- arrow::read_parquet("dat/adu_bmi_sei_dat.parquet")
dat_adu_bmi_sei
...
mod_adu_bmi_sei <- 
  glmmTMB(
    bmi ~ waveC + sex + (age_cat + sei + prs)^2 + 
      (1 + waveC | hicid) + (1 | personid),
    family = gaussian(), 
    data = dat_adu_bmi_sei,
    REML = TRUE
    # verbose = TRUE
  )
...
# lvls_age_cat <- levels(dat_adu_bmi_sei$age_cat)
...
newdat_base <- dat_adu_bmi_sei 
# newdat_base <- newdat_base %>% mutate(sex = 0.5, waveC = 3L)
newdat_base <- newdat_base %>% mutate(sex = 0.5)
newdat_basep <- predict(mod_adu_bmi_sei, newdata = newdat_base, type = "response", allow.new.levels = FALSE, re.form = NULL)
# newdat_basep <- predict(mod_adu_bmi_sei, type = "response", allow.new.levels = TRUE)
...
newdat_base_pred <- 
  predict(
    mod_adu_bmi_sei, 
    newdata = dat_adu_bmi_sei, 
    type = "response", 
    allow.new.levels = FALSE, 
    re.form=NULL
  )
newdat_base <- 
  cbind(dat_adu_bmi_sei, pred_bmi = newdat_base_pred) %>%
  group_by(sei, prs, age_cat, sex) %>%
  summarise(pred_p = mean(pred_bmi), .groups = "drop") %>%
  arrange(sei, prs, age_cat, sex)

# test
get_pred_by_cats(mod_adu_bmi_sei, dat_adu_bmi_sei)
get_pred_by_cats(mod_adu_bmi_sei, dat_adu_bmi_sei) %>%
  full_join(
    .,
    newdat_base,
...
all(
  abs(
    get_pred_by_cats(fit_glmm_tmb(dat_adu_bmi_sei), dat_adu_bmi_sei)$pred_p - 
      newdat_base$pred_p
  ) < 1e-9
)
